...
.EXAMPLE
    .\generateBellows.ps1 -top_radius 10 -bottom_radius 50 -total_height 120 -offset 15 -min_step_height 5 -decimals 2 -amplitudeFactor 0.8 -ascii -exportSCAD ".\bellow.scad" -exportCSV ".\bellow_profile.csv"
...
function New-BellowProfile {
    ...
    $cycleH = $growH + $shrinkH  # one full corrugation cycle

    # Figure out how many cycles fit in usable height
    $numCycles = [math]::Floor($usable_height / $cycleH)

    # Recompute actual deltaH per cycle to fit evenly
    $cycleH = $usable_height / $numCycles
    $growH = $cycleH * ($growH / ( $growH + $shrinkH ))
    $shrinkH = $cycleH * ($shrinkH / ( $growH + $shrinkH ))

    # Amplitude
    $amplitudeG = $growH * $amplitudeFactor
    $amplitudeS = $shrinkH * $amplitudeFactor
    ...
    for ($k = 0; $k -le $cycleH; $k++) {
        ...
        if ($k % 2 -eq 1) {
            ...
            #Write-Host "Shrink: $baseline"
            #$r = $baseline - $amplitudeS
            $r = $r - $amplitudeS
            if (($r -le $min_bellow_radius + 1e-9) -or ($h -ge $total_height - $offset) ) {
                # Land on top radius and stop — we still end on a Shrink as desired
                Add-Point ("Shrink#{0}" -f [int][math]::Ceiling($k / 2)) $top_radius ($total_height - $offset)
                $stopped = $true
                break
            }
            ...
            $h += $growH
            #$baseline = Get-Baseline $h
            # Write-Host "Grow: $baseline"
            #$r = $baseline + $amplitudeG
            $r = $r + $amplitudeG
            if ($h -ge $total_height - $cycleH - $offset) {
                # Land on top radius and stop — we still end on a Shrink as desired
                Add-Point ("Grow#{0}" -f [int][math]::Ceiling($k / 2)) $top_radius ($total_height - $offset)
                $stopped = $true
                break
            }
